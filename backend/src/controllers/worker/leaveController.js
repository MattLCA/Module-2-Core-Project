...
import { createForRole } from "../../models/notificationModel.js";
  ...
  try {
    ...
    if (!employeeId) {
      return res.status(401).json({
        error: "Authenticated worker ID is missing.",
      });
    }
    ...
    if (!Number.isInteger(requestedDays) || requestedDays < 1) {
      return res.status(400).json({
        message: "Leave days must be a whole number of at least 1.",
      });
    }
    ...
    if (calculatedDays === null) {
      return res.status(400).json({
        message: "Please enter a valid leave date range.",
      });
    }

    if (requestedDays !== calculatedDays) {
      return res.status(400).json({
        message: `The selected dates equal ${calculatedDays} day(s), but ${requestedDays} day(s) were requested.`,
      });
    }
    ...
    const leaveType = await getLeaveTypeById(Number(leaveTypeId));
    ...
    if (!balance) {
      return res.status(400).json({
        message: `No current-year balance exists for ${leaveType.leaveTypeName}.`,
      });
    }
    ...
    if (requestedDays > remainingDays) {
      return res.status(400).json({
        message: `You only have ${remainingDays} day(s) of ${leaveType.leaveTypeName} remaining.`,
      });
    }
    ...
    try {
      const notifications = await createForRole({
        ...
        message: `${leaveType.leaveTypeName} leave request submitted by employee ${employeeId}.`,
      ...
      console.error("[Worker Leave] Failed to notify HR:", notificationError);
    }
