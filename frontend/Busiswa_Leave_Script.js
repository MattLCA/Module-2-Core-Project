...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (
    ...
    console.error("[HR Leave] Required leave page elements are missing.");
    return;
  }
  ...
  async function apiRequest(endpoint, options = {}) {
    ...
    if (!token) {
      console.error("[HR Leave] No HR authentication token found.");
    ...
    try {
      ...
      console.error("[HR Leave] Backend connection failed:", error);
    ...
    if (contentType && contentType.includes("application/json")) {
      try {
        ...
        console.error("[HR Leave] Could not parse API response:", error);
      }
    }
  ...
  async function loadLeaveRequests() {
    try {
      ...
      console.log("[HR Leave] Requests returned by backend:", leaveRequests);

      renderLeaveRows();
    } catch (error) {
      console.error("[HR Leave] Failed to load leave requests:", error);
  ...
  function getFilteredRequests() {
    const searchValue = leaveSearch.value.trim().toLowerCase();

    const departmentValue = leaveDepartment.value;

    const typeValue = leaveType.value;

    const statusValue = leaveStatus.value;

    return leaveRequests.filter((request) => {
      const employee = String(request.employeeFullName || "").toLowerCase();

      const employeeCode = String(request.employeeCode || "").toLowerCase();

      const department = String(request.department || "");

      const requestLeaveType = String(request.leaveType || "");

      const status = String(request.leaveStatus || "");
      ...
      const matchesDepartment =
        departmentValue === "All Departments" || department === departmentValue;

      const matchesType =
        typeValue === "All Types" || requestLeaveType === typeValue;

      const matchesStatus =
        statusValue === "All Statuses" || status === statusValue;
  ...
  function renderLeaveRows() {
    ...
    filtered.forEach((request) => {
      ...
      const employeeName = request.employeeFullName || "Unknown employee";
      ...
      const requestId =
        request.requestId ?? request.leaveRequestId ?? request.leave_request_id;
      ...
      row.innerHTML = `
                ...
                <td>
                    ${escapeHtml(request.endDate || "-")}
                </td>

                <td>
                    ${escapeHtml(formatDuration(request.duration))}
                </td>
                ...
                <td>
                    ${escapeHtml(request.submittedDate || "-")}
                </td>

                <td>
                    ${escapeHtml(request.reviewerName || "-")}
                </td>
  ...
  function openModal(request, action) {
    if (
      ...
      console.error("[HR Leave] Decision modal elements are missing.");
    ...
    modalSubtitle.textContent = isApprove
    ...
    modalRequestDetails.innerHTML = `
            <strong>
                ${escapeHtml(request.employeeFullName || "Unknown employee")}
            </strong>

            <br>

            Employee ID:
            ${escapeHtml(request.employeeCode || request.employeeId || "-")}
    ...
    if (modalReasonTextarea && modalTextareaLabel) {
  ...
  async function processDecision() {
    ...
    if (!Number.isInteger(requestId) || requestId <= 0) {
      console.error("[HR Leave] Invalid leave request ID:", activeRequest);

      alert("This leave request does not have a valid database ID.");

      return;
    }

    const status = activeAction === "approve" ? "Approved" : "Rejected";

    let reason = "";

    if (status === "Rejected") {
      reason = modalReasonTextarea ? modalReasonTextarea.value.trim() : "";
    ...
    console.log("[HR Leave] Sending leave decision:", {
      requestId,
      status,
      reason,
    });
    ...
    try {
      const response = await apiRequest(`/leave/${requestId}/decision`, {
        method: "PUT",
      ...
      console.log("[HR Leave] Decision response:", response);
      ...
      if (response?.data?.notificationCreated === false) {
        alert(
          status === "Approved"
      ...
      console.error("[HR Leave] Decision request failed:", error);

      alert(error.message || "Could not update the leave request.");
    } finally {
      if (modalConfirmBtn) {
  ...
  leaveStatus.addEventListener("change", renderLeaveRows);
  ...
  if (modalConfirmBtn) {
    modalConfirmBtn.addEventListener("click", processDecision);
  }
