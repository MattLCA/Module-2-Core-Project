...
export const getPayslips = async (req, res) => {
    try {
        ...
        const payslips = await getPayslipsByEmployeeId(
            employeeId
        );
...
export const downloadPayslip = async (req, res) => {
    try {
        ...
        doc
            .font('Helvetica')
            .text(
                `Base Salary: R ${formatMoney(payslip.baseSalary)}`
            )
            .text(
                `Hours Worked: ${formatNumber(payslip.hoursWorked)}`
            )
            .text(
                `Leave Deductions: R ${formatMoney(payslip.leaveDeductions)}`
            );
        ...
        doc
            ...
            .font('Helvetica-Bold')
            .text(
                `FINAL SALARY: R ${formatMoney(payslip.finalSalary)}`,
                {
                    align: 'right'
                }
            );
