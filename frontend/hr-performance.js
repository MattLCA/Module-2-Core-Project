document.addEventListener("DOMContentLoaded", () => {
  ...
  (function setDateAndCycle() {
    ...
    if (dateText)
      dateText.textContent = `${dateStr} · Q${quarter} ${now.getFullYear()} review cycle`;
    if (reviewCycleSub)
      reviewCycleSub.textContent = `Q${quarter} ${now.getFullYear()} review cycle`;
  })();
  ...
  function mapPerformanceRecord(r) {
    return {
      ...
      goalProgress:
        r.goal_progress !== null && r.goal_progress !== undefined
          ? Number(r.goal_progress)
          : null,
    };
  }
  ...
  function loadPerformance() {
    ...
    return apiFetch("/performance")
      ...
      .catch(function (err) {
        if (performanceTbody) {
          performanceTbody.innerHTML =
            '<tr><td colspan="6">Couldn\u2019t load performance data: ' +
            err.message +
            "</td></tr>";
        }
  ...
  function renderPerformanceTable() {
    ...
    performanceTbody.innerHTML = employees
      .map((e) => {
        ...
        const goalProgress =
          e.goalProgress !== null && e.goalProgress !== undefined
            ? e.goalProgress
            : 0;
    ...
    if (teamPerformanceSub)
      teamPerformanceSub.textContent = `${employees.length} team members`;
  ...
  function renderNotifications(notifications) {
    ...
    if (!notifications.length) {
      notifList.innerHTML =
        '<li class="notif-item"><p>No notifications yet.</p></li>';
    } else {
  ...
  function loadNotifications() {
    return apiFetch("/notifications")
      ...
      .catch(() => {
        if (notifList)
          notifList.innerHTML =
            '<li class="notif-item"><p>Couldn\u2019t load notifications.</p></li>';
      });
  }
    ...
    notifList.addEventListener("click", (e) => {
      ...
      apiFetch(`/notifications/${id}/read`, { method: "PATCH" })
        .then(() => {
          ...
          const remaining =
            notifList.querySelectorAll(".notif-item.unread").length;
  ...
  const performanceEmptyState = document.getElementById(
    "performanceEmptyState",
  );
  const performanceTable = performanceEmptyState
    ? performanceEmptyState.previousElementSibling
    : null;
  ...
  if (searchInput) {
    searchInput.addEventListener("input", (e) => {
      ...
      if (performanceTable)
        performanceTable.style.display = visible === 0 ? "none" : "";
    });
  }
  ...
  function recalcStats() {
    ...
    const overdueRows = rows.filter((r) =>
      r.querySelector(".status-pill")?.classList.contains("overdue"),
    );

    if (avgRatingVal) {
      if (ratedRows.length) {
        const avg =
          ratedRows.reduce((sum, r) => sum + parseFloat(r.dataset.rating), 0) /
          ratedRows.length;
    ...
    if (reviewsCompletedVal)
      reviewsCompletedVal.textContent = String(ratedRows.length);
    if (overdueReviewsVal)
      overdueReviewsVal.textContent = String(overdueRows.length);
  }
    ...
    apiFetch("/performance/" + employeeId, {
      ...
      .then(function (result) {
        ...
        const idx = employees.findIndex(
          (emp) => emp.employeeId === updated.employeeId,
        );
  ...
  const closeViewReviewFooterBtn = document.getElementById(
    "closeViewReviewFooterBtn",
  );
  ...
  function openViewReviewModal(row) {
    ...
    viewReviewRating.textContent = row.dataset.rating
      ? parseFloat(row.dataset.rating).toFixed(1)
      : "\u2014";
    viewReviewDate.textContent =
      row.querySelector(".last-review-cell").textContent;
    viewReviewNotes.textContent =
      row.dataset.notes && row.dataset.notes.length
        ? row.dataset.notes
  ...
  if (closeViewReviewBtn)
    closeViewReviewBtn.addEventListener("click", closeViewReviewModal);
  if (closeViewReviewFooterBtn)
    closeViewReviewFooterBtn.addEventListener("click", closeViewReviewModal);
  viewReviewModal?.addEventListener("click", (e) => {
    if (e.target === viewReviewModal) closeViewReviewModal();
  });
  ...
  document
    .getElementById("performanceTbody")
  ...
  if (scheduleOneOnOneBtn)
    scheduleOneOnOneBtn.addEventListener("click", openScheduleModal);
  ...
  scheduleForm?.addEventListener("submit", (e) => {
    ...
    const timeLabel = new Date(`2000-01-01T${time}`).toLocaleTimeString(
      "en-US",
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if (viewReviewModal && !viewReviewModal.hidden) {
      closeViewReviewModal();
      return;
    }
  ...
  function loadReviewCycleFunnel() {
    return apiFetch("/review-cycle")
      .then((result) => {
        ...
        if (!data) {
          if (reviewCycleSub)
            reviewCycleSub.textContent = "No active review cycle";
          return;
        }
        const { funnel } = data;
        setFunnelBar(
          "funnelSelfReviewCount",
          "funnelSelfReviewFill",
          funnel.selfReviewSubmittedPct,
        );
        setFunnelBar(
          "funnelManagerReviewCount",
          "funnelManagerReviewFill",
          funnel.managerReviewSubmittedPct,
        );
        setFunnelBar(
          "funnelCalibrationCount",
          "funnelCalibrationFill",
          funnel.calibrationCompletePct,
        );
        setFunnelBar(
          "funnelFinalizedCount",
          "funnelFinalizedFill",
          funnel.finalizedPct,
        );
      })
  ...
  function renderGoals(goals) {
    ...
    if (goalsSub)
      goalsSub.textContent = `${goals.length} goal${goals.length === 1 ? "" : "s"}`;
  }
  ...
  function loadGoalsSummary() {
    return apiFetch("/goals/summary")
      .then((result) => {
        ...
        if (goalsOnTrackVal)
          goalsOnTrackVal.textContent =
            percentOnTrack === null ? "\u2014" : `${percentOnTrack}%`;
      })
      .catch(() => {
        if (goalsOnTrackVal) goalsOnTrackVal.textContent = "\u2014";
      });
  }
