  ...
  function renderNotifications(notifications) {
    ...
    if (!notifications.length) {
      ...
      notifList.innerHTML = notifications.map(n => `
        <li class="notif-item ${n.isRead ? '' : 'unread'}" data-notification-id="${n.notificationId}">
          <i class="ti ${n.isRead ? 'ti-bell' : 'ti-alert-circle'}" aria-hidden="true"></i>
          <div><p>${n.title ? n.title + ' — ' : ''}${n.message}</p><span>${timeAgo(n.createdAt)}</span></div>
        </li>
      `).join('');
    }
