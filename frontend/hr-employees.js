(function () {
  ...
  function statusLabel(status) {
    ...
    if (status === "onboarding")
      return { cls: "onboarding", text: "Onboarding" };
  ...
  function renderRows(rows) {
    if (!rows.length) {
      tbody.innerHTML =
        '<tr class="empty-row"><td colspan="6">No employees match your search or filter.</td></tr>';
      return;
    }
    var html = "";
    rows.forEach(function (e, idx) {
      ...
      html +=
        "" +
        "<tr>" +
        '<td><div class="emp-cell"><div class="emp-avatar">' +
        initials(e.name) +
        '</div><div><div class="emp-name">' +
        e.name +
        '</div><div class="emp-email">' +
        e.email +
        "</div></div></div></td>" +
        '<td><span class="dept-tag"><span class="dept-dot" style="background:' +
        color +
        ';"></span>' +
        e.dept +
        "</span></td>" +
        ...
        '<button class="row-menu" data-row="' +
        idx +
        '" aria-label="Row actions"><i class="ti ti-dots"></i></button>' +
        '<div class="row-dropdown" data-dropdown="' +
        idx +
        '">' +
        '<button type="button" data-action="view" data-row="' +
        idx +
        '"><i class="ti ti-eye"></i>View profile</button>' +
        '<button type="button" data-action="edit" data-row="' +
        idx +
        '"><i class="ti ti-edit"></i>Edit details</button>' +
        '<button type="button" class="danger" data-action="remove" data-row="' +
        idx +
  ...
  function renderPager(totalPages) {
    var html = "";
    html +=
      '<button class="page-btn" id="pagerPrev" ' +
    ...
    html +=
      '<button class="page-btn" id="pagerNext" ' +
  ...
  function updateSummary() {
    ...
    document.getElementById("statActive").textContent = employees.filter(
      function (e) {
        return e.status === "active";
      },
    ).length;
    document.getElementById("statOnboarding").textContent = employees.filter(
      function (e) {
    ...
    if (subEl) {
      subEl.textContent =
        employees.length + " people across " + deptCount + " departments";
    }
  }
  ...
  function loadEmployees() {
    tbody.innerHTML =
      '<tr class="empty-row"><td colspan="6">Loading employees\u2026</td></tr>';
    ...
    apiFetch("/employees")
      ...
      .catch(function (err) {
        tbody.innerHTML =
          '<tr class="empty-row"><td colspan="6">Couldn\u2019t load employees: ' +
          err.message +
          "</td></tr>";
  ...
  sortChip.addEventListener("click", function () {
    ...
    sortChip.innerHTML =
      '<i class="ti ti-adjustments-horizontal"></i>Sort: name ' +
      (state.sortDir === "asc" ? "\u2191" : "\u2193");
    render();
  });
  ...
  tbody.addEventListener("click", function (e) {
    ...
    if (actionBtn) {
      ...
      var visibleRows = getFiltered().slice(
      ...
      if (action === "remove") {
        if (
          !confirm(
            "Remove " +
              employee.name +
              " from the directory? This can be undone by an admin later (soft delete).",
          )
        )
          return;
    ...
    apiFetch("/employees/" + employeeId)
      .then(function (result) {
        ...
        viewModalBody.innerHTML =
          '<div class="profile-row"><span class="profile-label">Name</span><span>' +
          escapeHtml(e.name) +
          "</span></div>" +
          '<div class="profile-row"><span class="profile-label">Email</span><span>' +
          escapeHtml(e.email || "—") +
          "</span></div>" +
          '<div class="profile-row"><span class="profile-label">Employee code</span><span>' +
          escapeHtml(e.employee_code) +
          "</span></div>" +
          '<div class="profile-row"><span class="profile-label">Department</span><span>' +
          escapeHtml(e.department) +
          "</span></div>" +
          '<div class="profile-row"><span class="profile-label">Role</span><span>' +
          escapeHtml(e.position) +
          "</span></div>" +
          '<div class="profile-row"><span class="profile-label">Base salary</span><span>R ' +
          Number(e.base_salary).toLocaleString("en-ZA") +
          "</span></div>" +
          '<div class="profile-row"><span class="profile-label">Contact</span><span>' +
          escapeHtml(e.contact || "—") +
          "</span></div>" +
          '<div class="profile-row"><span class="profile-label">Status</span><span>' +
          (e.is_active ? "Active" : "Inactive") +
          "</span></div>";
      })
      .catch(function (err) {
        viewModalBody.innerHTML =
          "<p>Couldn\u2019t load profile: " + escapeHtml(err.message) + "</p>";
      });
  }
  ...
  document
    .getElementById("viewModalCloseBtn")
    .addEventListener("click", closeViewModal);
