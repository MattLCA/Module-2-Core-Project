document.addEventListener("DOMContentLoaded", () => {
    ...
    function buildRecordFromApi(p) {
        const hoursWorked = Number(p.hoursWorked) || 0;
        const leaveDeductions = Number(p.leaveDeductions) || 0;
        const finalSalary = Number(p.finalSalary) || 0;
        const baseSalary = Number(p.baseSalary) || finalSalary;

        // leaveDeductions is a Rand amount (already folded into finalSalary),
        // not a number of hours — it must not be subtracted from hoursWorked.
    ...
    function loadPayrollRecords() {
        ...
        return apiFetch('/payroll')
            .then(function(result) {
                const records = (result.data || []).map(buildRecordFromApi);

                // The API returns every payroll run ever generated (all pay
                // periods for all employees). This page shows one run at a
                // time, so keep only the most recent pay period present.
                if (!records.length) return records;
                const latestPeriod = records.reduce(
                    (max, r) => (r.payPeriod > max ? r.payPeriod : max),
                    records[0].payPeriod
                );
                return records.filter(r => r.payPeriod === latestPeriod);
            })
            .catch(function(err) {
