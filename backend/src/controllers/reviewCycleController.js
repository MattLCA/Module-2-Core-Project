...
async function updateProgress(req, res, next) {
  try {
    ...
    if (!cycle) {
      throw new ApiError(404, "No active review cycle");
    }
    ...
    if (selfReviewSubmitted !== undefined) {
      flags.self_review_submitted = selfReviewSubmitted;
    }

    if (managerReviewSubmitted !== undefined) {
      flags.manager_review_submitted = managerReviewSubmitted;
    }

    if (calibrationComplete !== undefined) {
      flags.calibration_complete = calibrationComplete;
    }
